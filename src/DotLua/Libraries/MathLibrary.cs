...
namespace DotLua.Libraries
{
    ...
    {
        private static LuaArguments math_abs(LuaArguments args)
        {
            return Lua.Return(Math.Abs(args[0]));
        }

        private static LuaArguments math_acos(LuaArguments args)
        {
            return Lua.Return(Math.Acos(args[0]));
        }

        private static LuaArguments math_asin(LuaArguments args)
        {
            return Lua.Return(Math.Asin(args[0]));
        }

        private static LuaArguments math_atan(LuaArguments args)
        {
            return Lua.Return(Math.Atan(args[0]));
        }

        private static LuaArguments math_atan2(LuaArguments args)
        {
            return Lua.Return(Math.Atan2(args[0], args[1]));
        }

        private static LuaArguments math_ceil(LuaArguments args)
        {
            return Lua.Return(Math.Ceiling(args[0]));
        }

        private static LuaArguments math_cos(LuaArguments args)
        {
            return Lua.Return(Math.Cos(args[0]));
        }

        private static LuaArguments math_cosh(LuaArguments args)
        {
            return Lua.Return(Math.Cosh(args[0]));
        }

        private static LuaArguments math_exp(LuaArguments args)
        {
            return Lua.Return(Math.Exp(args[0]));
        }

        private static LuaArguments math_floor(LuaArguments args)
        {
            return Lua.Return(Math.Floor(args[0]));
        }

        private static LuaArguments math_log(LuaArguments args)
        {
            return Lua.Return(Math.Log(args[0], args[1] | Math.E));
        }

        private static LuaArguments math_max(LuaArguments args)
        {
            var max = args[0];
            max = args.Aggregate(max, (current, o) => Math.Max(current, o));
            return Lua.Return(max);
        }

        private static LuaArguments math_min(LuaArguments args)
        {
            var min = args[0];
            min = args.Aggregate(min, (current, o) => Math.Min(current, o));
            return Lua.Return(min);
        }

        private static LuaArguments math_pow(LuaArguments args)
        {
            return Lua.Return(Math.Pow(args[0], args[1]));
        }

        private static LuaArguments math_sin(LuaArguments args)
        {
            return Lua.Return(Math.Sin(args[0]));
        }

        private static LuaArguments math_sinh(LuaArguments args)
        {
            return Lua.Return(Math.Sinh(args[0]));
        }

        private static LuaArguments math_sqrt(LuaArguments args)
        {
            return Lua.Return(Math.Sqrt(args[0]));
        }

        private static LuaArguments math_tan(LuaArguments args)
        {
            return Lua.Return(Math.Tan(args[0]));
        }

        private static LuaArguments math_tanh(LuaArguments args)
        {
