...
namespace DotLua
{
    ...
    {
        private readonly List<LuaObject> list;
    ...
    public class LuaObject : DynamicObject, IEnumerable<KeyValuePair<LuaObject, LuaObject>>
    {
        ...
        /// </summary>
        public LuaObject Metatable { get; set; } = Nil;
        ...
        {
            return Call(new LuaArguments(args));
        }
        ...
        ///     And empty string
        /// </summary>
        public static readonly LuaObject EmptyString = new LuaObject
        {
            luaobj = "",
            type = LuaType.@string,
            Metatable = Nil
        ...
        public bool IsBool
        {
            get { return type == LuaType.boolean; }
        }
        ...
        {
            get { return type == LuaType.number; }
        }
        ...
        {
            get { return type == LuaType.@string; }
        }
        ...
        {
            get { return type == LuaType.function; }
        }
        ...
        {
            get { return type == LuaType.userdata; }
        }
        ...
        {
#if DNXCORE50
            LuaObject[] passingArgs = args.Select(x => (x is LuaObject ? (LuaObject)x : LuaObject.FromObject(x))).ToArray();
#else
            var passingArgs = Array.ConvertAll(args,
                x => (x is LuaObject ? (LuaObject)x : FromObject(x)));
#endif
            var ret = Call(passingArgs);
            if (ret.Length == 1)
            {
                if (ret[0].IsNil)
                    result = null;
...
#if DNXCORE50
                object[] res = ret.Select(x => getObject(x)).ToArray();
#else
            var res = Array.ConvertAll(ret.ToArray(), x => getObject(x));
#endif
            result = res;
            return true;
        }
