    ...
    internal class GrammarDataBuilder
    {
        ...
        private static bool ComputeNullability(NonTerminal nonTerminal)
        {
            foreach (var prod in nonTerminal.Productions)
            {
                ...
                foreach (var child in prod.RValues)
                {
        ...
        private void ValidateGrammar()
        {
                ...
                foreach (var prod in nt.Productions)
                    if (prod.Flags.IsSet(ProductionFlags.IsError))
                    {
                        ...
                        if (!(lastTerm is Terminal) || lastTerm == _grammar.SyntaxError)
                            _language.Errors.Add(GrammarErrorLevel.Warning, null, Resources.ErrLastTermOfErrorProd,
                                nt.Name);
            ...
            if (invalidTransSet.Count > 0)
                _language.Errors.Add(GrammarErrorLevel.Error, null, Resources.ErrTransientNtMustHaveOneTerm,
