    ...
    internal class ParserDataBuilder
    {
        ...
        private void CreateParserStates()
        {
            ...
            foreach (var augmRoot in grammarData.AugmentedSnippetRoots)
            {
                ...
                ExpandParserStateList(_data.States.Count - 1);
                    //start with just added state - it is the last state in the list
        ...
        private void ComputeLookaheads(LRItemSet forItems)
        {
            foreach (var reduceItem in forItems)
            {
                ...
                if (reduceItem.Lookaheads.Count == 0)
                    _language.Errors.Add(GrammarErrorLevel.InternalError, reduceItem.State,
        ...
        private void HandleUnresolvedConflicts()
        {
            foreach (var state in _data.States)
            {
                ...
                if (shiftReduceConflicts.Count > 0)
                    _language.Errors.Add(GrammarErrorLevel.Conflict, state, Resources.ErrSRConflict, state,
                        shiftReduceConflicts.ToString());
                if (reduceReduceConflicts.Count > 0)
                    _language.Errors.Add(GrammarErrorLevel.Conflict, state, Resources.ErrRRConflict, state,
                        reduceReduceConflicts.ToString());
        ...
        private void CreateRemainingReduceActions()
        {
            foreach (var state in _data.States)
            {
                ...
                if (stateData.ShiftItems.Count == 0 && stateData.ReduceItems.Count == 1)
                {
                    ...
                    continue;
                        //next state; if we have default reduce action, we don't need to fill actions dictionary for lookaheads
                }
                //create actions
