...
namespace Irony.Parsing
{
    ...
    public class ParsingContext
    {
        ...
        public void AddTrace(bool asError, string message, params object[] args)
        {
            ...
            if (args != null && args.Length > 0)
                message = string.Format(message, args);
            ParserTrace.Add(new ParserTraceEntry(CurrentParserState, ParserStack.Top, CurrentParserInput, message,
                asError));
        }
        ...
        public StringSet GetExpectedTermSet()
        {
            ...
            if (CurrentParserState.ReportedExpectedSet == null)
                CurrentParserState.ReportedExpectedSet =
                    ParserDataBuilder.ComputeGroupedExpectedSetForState(Language.Grammar, CurrentParserState);
    ...
    public struct VsScannerStateMap
    {
        ...
        [FieldOffset(0)] public byte TerminalIndex; //1-based index of active multiline term in MultilineTerminals
        [FieldOffset(1)] public byte TokenSubType; //terminal subtype (used in StringLiteral to identify string kind)
        [FieldOffset(2)] public short TerminalFlags; //Terminal flags
    } //struct
}