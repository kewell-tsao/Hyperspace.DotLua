...
namespace Irony.Parsing
{
    ...
    ///     activated during parser construction. The hint code analyzes the conflict and resolves it by adding custom or
    ///     general action
    ...
    public class PrecedenceHint : GrammarHint
    {
        public override void Apply(LanguageData language, LRItem owner)
        {
            ...
            foreach (var conflict in operConflicts)
            {
                ...
                state.Actions[conflict] = new PrecedenceBasedParserAction(conflict, newState,
                    reduceItems[0].Core.Production);
